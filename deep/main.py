...
def main(csv_file_path):
    ...
    train_dataloader = DataLoader(
        train_dataset, batch_size=params["BATCH_SIZE"], shuffle=False, drop_last=True, num_workers=1
    )

    val_dataset = CryptoDataset(val, params["SEQ_LENGTH"], params["FEATURES"], params["TARGET"])
    val_dataloader = DataLoader(
        val_dataset, batch_size=params["BATCH_SIZE"], shuffle=False, drop_last=True, num_workers=1
    )
    ...
    model = LSTM(
        params["NUM_FEATURES"], params["HIDDEN_SIZE"], params["NUM_LAYERS"], params["OUTPUT_SIZE"], params["DROPOUT"]
    ).to(device)
